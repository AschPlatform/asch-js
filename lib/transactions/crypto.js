...
function getBytes_(transaction, skipSignature, skipSecondSignature) {
	var assetSize = 0,
		assetBytes = null;
...
function sign(transaction, keys) {
	...
	if (!transaction.signature) {
		transaction.signatures = [new Buffer(signature).toString("hex")];
	} else {
...
function secondSign(transaction, keys) {
	var hash = getHash(transaction, true, true);
	var signature = nacl.sign.detached(hash, new Buffer(keys.privateKey, "hex"));
	transaction.secondSignature = new Buffer(signature).toString("hex")
}
